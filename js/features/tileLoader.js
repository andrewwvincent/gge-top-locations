        ...
        if (!map.getSource(sourceId)) {
            ...
            map.addLayer({
                ...
                'paint': {
                    ...
                    'fill-opacity': 0.8,
                    'fill-outline-color': config.debug.showTileFilenames ? '#000000' : 'rgba(0, 0, 0, 0)'
                }
            }, labelLayer ? labelLayer.id : undefined);

            // Add outline layer for debug mode
            if (config.debug.showTileFilenames) {
            ...
            // Add debug text layer if enabled
            if (config.debug.showTileFilenames && metadata && metadata.grids && metadata.grids[gridRef]) {
                // Calculate center point of the tile's bounds
                const bounds = metadata.grids[gridRef].bounds;
                const centerLat = (bounds.min_lat + bounds.max_lat) / 2;
                const centerLon = (bounds.min_lon + bounds.max_lon) / 2;

                // Add a GeoJSON source for the text
                const textSourceId = `text-source-${gridRef}`;
                map.addSource(textSourceId, {
                    ...
                    'data': {
                        ...
                        'properties': {
                            'text': `${gridRef}.geojson`
                        }
                    }
                });

                // Add a symbol layer for the text
                const textLayerId = `text-layer-${gridRef}`;
                map.addLayer({
                    'id': textLayerId,
                    'type': 'symbol',
                    'source': textSourceId,
...
async function unloadTile(gridRef) {
    const sourceId = `source-${gridRef}`;
    const layerId = `layer-${gridRef}`;
    const textSourceId = `text-source-${gridRef}`;
    const textLayerId = `text-layer-${gridRef}`;
    const outlineLayerId = `outline-layer-${gridRef}`;
    ...
    try {
        ...
        // Remove text layer and source if they exist
        if (map.getLayer(textLayerId)) {
            map.removeLayer(textLayerId);
        }
        if (map.getSource(textSourceId)) {
            map.removeSource(textSourceId);
        }

        // Remove main layer and source
        if (map.getLayer(layerId)) {
            map.removeLayer(layerId);
        }
        if (map.getSource(sourceId)) {
        ...
        console.log(`Successfully unloaded ${gridRef}`);
    } catch (error) {
